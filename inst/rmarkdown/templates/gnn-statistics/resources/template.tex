...
% Note that the theming is hard-coded.
$if(fontfamily)$
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% See: https://tex.stackexchange.com/questions/47576/combining-ifxetex-and-ifluatex-with-the-logical-or-operation
% If, \ifxetex is true, then 0\ifxetex becomes 01, otherwise it is 00,
% and the first part of the if statement returns 1. Otherwise, if
% \ifluatex is true then the second part will return 1. Then it tests
% against 0. So if either are true, then the next two lines don't run.
% If both are false, then it means taht pdftex is the engine and the
% next two lines will run.
\ifnum 0\ifxetex 1\fi\ifluatex 1\fi=0 % if pdftex is the engine do the following
  \usepackage[$if(fontenc)$$fontenc$$else$T1$endif$]{fontenc}
  \usepackage[utf8]{inputenc}
  $if(euro)$
    \usepackage{eurosym}
  $endif$
  \else % if luatex or xelatex do the following
    \ifxetex
      % \usepackage{mathspec}
    ...
    \else
      \usepackage{fontspec}
    \fi
  \defaultfontfeatures{Ligatures=TeX,Scale=MatchUppercase} % Was MatchLowercase but this is better for scaling the size of the inline math fonts.
  $for(fontfamilies)$
...
\setbeamertemplate{itemize subitem}{\raise1.0pt\hbox{\tiny\faCircleO}}
\setbeamertemplate{itemize subsubitem}{\hbox{\tiny\faSquareO}}
...
\setbeamertemplate{title page} {
	...
	\begin{tikzpicture}[remember picture, overlay]
	  \node[xshift = 3.5cm, yshift = -1cm] at (current page.center) {\(P(x|\mu,\sigma^2)=\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{(x-\mu)^2}{2\sigma^2}\right)\)};
	\end{tikzpicture}
...
%       \draw[fill=darkgrey] (0pt, 0pt) rectangle ++ (\insertframenumber / \inserttotalframenumber * \paperwidth, \progressbar@pbht);
%   \end{tikzpicture}%
% }

\addtobeamertemplate{headline} {
  % \begin{beamercolorbox}[wd=\paperwidth,ht=1ex,center,dp=0ex]{nagasakiBlue}%
